...
static const long long LIMIT = 1000000000000LL;
...
int main() {
    ...
    long long total_blocks = (LIMIT - 1) / BLOCK + 1;

    vector<long long> solutions;
    mutex sol_mutex;
    atomic<long long> blocks_done(0);
    atomic<long long> last_pct_printed(-5);

    #pragma omp parallel
    {
        vector<long long> rem(bsz), phi_v(bsz), sig_v(bsz);
        vector<long long> local_sols;

        #pragma omp for schedule(dynamic, 4)
        for (long long bi = 0; bi < total_blocks; bi++) {
            long long lo = 1 + bi * BLOCK;
            ...
            for (int p : small_primes) {
                ...
                if (p == 2) {
                    for (long long j = start; j < hi; j += 2) {
                        long long idx = j - lo;
                        int e = __builtin_ctzll((unsigned long long)rem[idx]);
                        rem[idx] >>= e;
                        if (e == 1) {
                            sig_v[idx] *= 3;
                        } else {
                ...
                for (long long j = start; j < hi; j += p) {
                    ...
                    if (e == 1) {
                        ...
                        long long pe = p;
                        for (long long k = 1; k < e; k++) pe *= p;
                        phi_v[idx] *= (pe / p) * (p - 1);
                        long long sc = 1 + p, pk = (long long)p * p;
                        for (long long k = 2; k <= e; k++) { sc += pk; pk *= p; }
                        sig_v[idx] *= sc;
                    }
                }
            }
            ...
            for (long long n = n_lo; n < n_hi; n++) {
                ...
                local_sols.push_back(n);
            }

            long long done = ++blocks_done;
            long long pct = done * 100 / total_blocks;
            long long prev = last_pct_printed.load();
            if (pct / 5 > prev / 5) {
                if (last_pct_printed.compare_exchange_strong(prev, pct)) {
        ...
        lock_guard<mutex> lk(sol_mutex);
        solutions.insert(solutions.end(), local_sols.begin(), local_sols.end());
    }
    ...
    for (long long n : solutions) {
        ...
        printf("%8lld %20lld %8lld %8lld %26lld %10lld\n",
            p, n, p + 2, p + 6, n + 12, r);
    }
    ...
    for (int i = 0; i < (int)solutions.size(); i++) {
        ...
        for (int pp : small_primes) {
            if ((long long)pp * pp > tmp) break;
